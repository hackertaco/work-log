...
export function buildExtractPayload(area, prompts, behavior = null) {
  ...
  const instruction =
    ...
    // 프롬프트는 대화의 한쪽이라 "무엇을 요구했나"만 보기 쉽다. 그런데 되묻는 방식 —
    // 흐린 답을 어디까지 파고드는지, 어떤 대답을 안 받아주는지 — 이 사람마다 확연히 다르고
    // 남이 따라 하기에도 유용한 정보다. 명시적으로 보라고 하지 않으면 통째로 놓친다.
    `무엇을 요구했는지뿐 아니라 "어떻게 되물었는지"도 보라 — 같은 걸 다시 묻거나, 두루뭉술한 ` +
    `답을 안 받아들이고 구체적인 것이 나올 때까지 파고들거나, 답을 듣고 전제를 바꾸는 패턴이 ` +
    `보이면 그것도 판단 기준으로 뽑아라. ` +
