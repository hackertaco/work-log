...
test("행동신호가 없을 때의 페이로드는 스냅샷과 글자 하나까지 같다", async () => {
  // 새 코드끼리 비교하면 두 쪽이 함께 틀어질 때 못 잡는다. 그래서 파일로 박아둔 스냅샷과 
  // 대조한다. 원본은 v2 직전 커밋(00084f4)에서 떴다. 이 테스트가 깨지면 신호 없는 경로가 
  // 바뀐 것이니, 의도한 변경일 때만 픽스처를 다시 뜨고 그 이유를 픽스처 _note 에 남긴다.
